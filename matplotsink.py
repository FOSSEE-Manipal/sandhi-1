...
#        self.hbox2.Add(self.ymax_control, border=5, flag=wx.ALL)
	...
	self.vbox = wx.BoxSizer(wx.VERTICAL)
        self.vbox.Add(self.canvas, 1, flag=wx.EXPAND | wx.ALL)        
...
#        	xmax = len(list(self.q)) if len(list(self.q)) > 50 else 50
#        else:
...
#                self.plot_data.set_xdata(np.arange(len(list(self.q.queue))))
#                self.plot_data.set_ydata(np.array(list(self.q.queue)))
#                self.axes.set_xlim(min(np.arange(len(list(self.q.queue)))),max(np.arange(len(list(self.q.queue)))))
